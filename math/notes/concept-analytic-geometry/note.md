...
이름값이 굳으면 **일화도 따라붙는다.** 천장을 기어가는 파리를 보다 좌표를 떠올렸다는 이야기가 좌표의 기원으로 가장 널리 돌지만, 그쪽은 **근거가 확인되지 않는다** — [[데카르트와 천장의 파리]]가 그 판정을 따로 맡는다. 거기 실린 도해는 **타일 몇 칸으로 한 자리를 가리키는 것**까지만 그리고, 이 노트 앞머리(§1 끝)의 **곡선을 식으로 옮기는** 그림과는 맡은 데가 다르다.
