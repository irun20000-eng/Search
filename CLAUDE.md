...
- `.github/workflows/gates.yml` — **서가 게이트 CI**(2026-09-09). 검증기 **아홉**을 PR·main push 에서 각각 별도 스텝으로 돌린다 — `tables`·`math`·`figures`·`roadmap`·`concept`·`guide`·`video`·`reports_meta`·`builders`. **수를 세지 말고 이름을 맞댈 것**(2026-09-21: 「일곱」인 채 **둘**이 빠져 있었다 — gates.yml 신설(#245) 때는 일곱이 맞았고 그 뒤 `reports_meta`(#248)·`roadmap`(#270) 가 늘 때 이 줄이 안 따라왔다. 수만 적으면 **무엇이 늘었는지** 못 본다). 「재발 방지는 문서가 아니라 게이트에 적는다」의 다음 단계 — 세션이 기억해야만 도는 검사는 문서지 게이트가 아니다. **도해 글자 계측의 기준 환경도 여기다**(개념 한 장이 2026-08-22 에 러너를 기준으로 삼은 것과 같은 이유).
