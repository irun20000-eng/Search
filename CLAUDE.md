...
- **비주얼 톤 = 라이트/웜화이트(#FFF8F0)+Nunito+코랄.** 다크 기본 가정 금지. 큰 시각 변경 전 Figma 시안 확인. 학습카드 임베드는 작게(기본 1장 4분할 요약).
  - **예외 — `math/` 수학사 섹션(2026-08-21 사용자 승인).** 고문서·아카이브 톤: 세리프 본문, 미색 지면(#FAF6EC), 잉크빛 텍스트, 시에나 강조(#9C5A2C), 유형 4색만 절제 사용. 라이트가 기본이고 **다크는 2026-08-21 사용자 승인으로 추가**했다(양피지 → 황혼: 지면 #1B1713, 잉크 #EDE3D2, 강조 #C88A52). SVG 도해는 지면색을 파일 안에 갖고 있어 CSS 변수가 닿지 않으므로 다크에서 `filter:brightness(.84)` 로 눌러 앉힌다. **도해 글자도 세리프 스택으로 통일했다**(2026-09-16 사용자 승인 — 그전까지 다섯 장이 산세리프였다). ⚠ 세리프에 숫자를 넣을 때 `font-variant-numeric:lining-nums` 를 **믿지 말 것** — 그 선언은 폰트가 `lnum` 피처를 가질 때만 듣고, 실측하니 EB Garamond(구글폰트)에도 NanumMyeongjo 에도 없다. 옛날식 숫자가 걱정되면 **그 숫자만 산세리프 `<tspan>`** 으로 뺀다. 상세·예외 등재는 `MATH_PIPELINE.md` §M3 과 `tools/verify_figures.py` 의 `FONT_EXCEPTIONS`. 사용자가 "기존 Search 톤 유지" 선택지를 보고 이 톤을 고른 의도적 예외이니 되돌리지 말 것. 목업 승인 절차는 그대로 지켰다(`math/_design/mockup-v1.html`).
...
- `.github/workflows/gates.yml` — **서가 게이트 CI**(2026-09-09). 검증기를 PR·main push 에서 각각 별도 스텝으로 돌린다 — `tables`·`math`·`figures`·`roadmap`·`concept`·`guide`·`video`·`reports_meta`·`manifest_paths`·`gate_list`·`builders`. **수를 세지 말고 이름을 맞댈 것**(2026-09-21: 「일곱」인 채 **둘**이 빠져 있었다 — gates.yml 신설(#245) 때는 일곱이 맞았고 그 뒤 `reports_meta`(#248)·`roadmap`(#270) 가 늘 때 이 줄이 안 따라왔다. 수만 적으면 **무엇이 늘었는지** 못 본다). **그래서 2026-09-22 에 수를 아예 지웠다** — 「아홉」이라 적혀 있던 자리에 `manifest_paths`(#294) 가 늘면서 또 어긋날 참이었다. 이름만 남기면 늘 때 이름을 더하는 것 말고는 할 일이 없다. 「재발 방지는 문서가 아니라 게이트에 적는다」의 다음 단계 — 세션이 기억해야만 도는 검사는 문서지 게이트가 아니다. **도해 글자 계측의 기준 환경도 여기다**(개념 한 장이 2026-08-22 에 러너를 기준으로 삼은 것과 같은 이유).
