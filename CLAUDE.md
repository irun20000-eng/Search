...
- **비주얼 톤 = 라이트/웜화이트(#FFF8F0)+Nunito+코랄.** 다크 기본 가정 금지. 큰 시각 변경 전 Figma 시안 확인. 학습카드 임베드는 작게(기본 1장 4분할 요약).
  - **예외 — `math/` 수학사 섹션(2026-08-21 사용자 승인).** 고문서·아카이브 톤: 세리프 본문, 미색 지면(#FAF6EC), 잉크빛 텍스트, 시에나 강조(#9C5A2C), 유형 4색만 절제 사용. 라이트가 기본이고 **다크는 2026-08-21 사용자 승인으로 추가**했다(양피지 → 황혼: 지면 #1B1713, 잉크 #EDE3D2, 강조 #C88A52). SVG 도해는 지면색을 파일 안에 갖고 있어 CSS 변수가 닿지 않으므로 다크에서 `filter:brightness(.84)` 로 눌러 앉힌다. **도해 글자도 세리프 스택으로 통일했다**(2026-09-16 사용자 승인 — 그전까지 다섯 장이 산세리프였다). 숫자는 `font-variant-numeric:lining-nums` 를 함께 걸 것 — 스택 앞머리(EB Garamond·Georgia)가 올드스타일 숫자를 쓴다. 사용자가 "기존 Search 톤 유지" 선택지를 보고 이 톤을 고른 의도적 예외이니 되돌리지 말 것. 목업 승인 절차는 그대로 지켰다(`math/_design/mockup-v1.html`).
