...
### 두 번째 길 — 배경을 코드로 그린다 (`scene`)

위 ②는 **사람 손에 묶여 있다.** 그림 생성이 사용자 계정(구글 Flow)에서만 되니, 그
한 칸이 비면 컷툰이 자리표로 멈춘다. 그래서 배경을 SVG 로 그리는 길을 하나 더 뒀다.

```python
{"n": 3, "cast": ["닥터파이"],
 "scene": "blackboard-pairing",                  # scenes.py 의 이름
 "figures": [{"src": "assets/characters/dr-pi.png", "h": 80}]}
```

인물은 이미 컷아웃으로 들어오므로 **배경만 있으면 컷이 선다.** `img` 와 `scene` 이
둘 다 있으면 `img` 가 이긴다 — Flow 그림이 오면 그것이 정본이라는 뜻이고, 그때까지
`scene` 이 자리를 지킨다.

덤이 하나 있다 — **글자가 정확하다.** 말풍선을 그림에 넣지 않는 이유가 생성 모델이
한글·숫자를 못 쓰기 때문인데, SVG 는 애초에 그 문제가 없다. 컷 3 칠판의
`2·4·8·16 / 1·2·3·4` 와 `×2`·`+1` 이 그 예다. 생성형으로는 못 얻는 컷이다.

**장면을 새로 그릴 때 지킬 것** — 실제로 걸렸던 것들이다.

- **viewBox 는 그 컷의 실제 픽셀 크기**로 잡는다. 렌더가 매번 컷 크기를 찍어 준다
  (`컷 크기 1128x430 · 556x341 …`). 그 값을 그대로 쓰면 좌표를 머리로 환산하지 않는다.
- **인물이 서는 자리에는 소품을 놓지 않는다.** 가운데 정렬(`center`)인 컷 한가운데에
  막대를 그렸다가 고양이 몸통을 그대로 관통했다.
- **말풍선·나레이션 자리를 비운다.** 불투명이라 가려지긴 하지만 그 밑이 빽빽하면
  컷이 답답해진다. 각 장면 함수 주석에 그 자리를 적어 둔다.
- **뜻 없는 소품은 뺀다.** 밀랍 봉인을 하나 놓아 봤는데 아무것도 설명하지 못해 지웠다.
- 색은 컷툰 CSS 의 팔레트(`--navy`·`--mustard`·`--mint`·`--cream`)만 쓴다.
