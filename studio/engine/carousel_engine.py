# -*- coding: utf-8 -*-
"""
카드뉴스 공장 엔진 (carousel factory) — Python 3.11 호환본

원본과의 유일한 차이: `_slide()` 안에 있던 '백슬래시 포함 중첩 f-string'을
3.11에서도 파싱되도록 일반 문자열 조립으로 바꿨다. (원본은 3.12+ 전용)
그 외 CONTENT · THEMES · BULK_CSS · 렌더 함수는 원본과 동일하다.
"""
import json, pathlib
...
def _dots(active):
    ...
    for i in range(1, 11):
        ...
        out.append('<span class="dot' + on + '"></span>')
    out.append('</div>')
    return "".join(out)

def _slide(i, theme, inner, dark, cover=False):
    cls = "slide dark" if dark else "slide"
    tw = "tickwrap " + theme["tick"] + (" cv" if cover else "")
    tick_html = '<div class="' + tw + '"><span class="tick"></span></div>'
    ghost = '' if cover else '<div class="ghost">%02d</div>' % i
    return ('<div class="' + cls + '" id="s' + str(i) + '"><div class="bgfx"></div>' + ghost +
            '<div class="pad">' + inner.replace("@TICK@", tick_html) + '</div></div>')
