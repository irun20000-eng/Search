...
- **게이트가 FAIL 해서 고쳤으면 빌더를 다시 돌릴 것 → 이제 게이트다: `python3 tools/verify_builders.py`.**
  ...
  **형태 통일(사람 몫)과 값 대조(기계 몫)를 하나로 묶어 값 대조까지 포기했던 것**이다.
  ...
  ⚠ **여전히 아무 게이트도 안 잡는 것**: `tldr`(본문 `## TL;DR` 절과 다른 문장이라 기준이 없다) ·
  손질 허용목록 16편의 **제목 내용** · `sources` 의 **표기 형태** · 최상위 `generated`(아무도 안 읽는다).
  표기 형태 통일은 발행물 표시가 바뀌는 일이라 **사람이 정할 자리**다 — 세션이 임의로 밀어붙이지 말 것.
