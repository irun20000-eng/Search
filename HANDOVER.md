...
### 2-9. 수학사 도해 부채 — **11장이 됐다, 여전히 얇다** (2026-09-08)

노트 88편 중 **자체 도해를 가진 것이 9편(SVG 11장)**이다. 이번 회차에 3장을 채웠다 —
`cavalieri-card-stack`(person-cavalieri §6, **인물 노트의 첫 도해**) ·
`hyperbola-equal-areas`(concept-logarithm-advanced §7) · `dice-sum-grid`(concept-probability §1).

- **대상 고르는 기준.** 「노트가 **이미 출처를 달고 말로 적어 둔 것** 중 그림이 대신 말할 수 있는 것」.
  새 사실을 들이지 않으므로 도해가 인용을 새로 요구하지 않는다. 이 기준을 벗어나면 그림 하나에
  수집 회차가 딸려 온다.
- **그림은 반드시 렌더해서 본다 — 그리고 렌더러는 Chromium 이다.** 이 러너에서 된다:
  `pip install playwright` 뒤 `executable_path='/opt/pw-browsers/chromium-1194/chrome-linux/chrome'`
  (버전이 안 맞아 `playwright install` 은 실패하므로 경로를 직접 준다).
  ⚠ **cairosvg 는 보기용으로만 쓸 것 — 판정에 쓰면 안 된다.** `text-anchor="middle"` + `<tspan>`
  에서 조각마다 따로 가운데 정렬해 **없는 겹침을 만들어 낸다**(LESSONS 2026-09-08 (2)).
- **11장 전부 눈으로 봤다 (2026-09-08).** 기존 8장에서 **결함 5건**이 나왔다 —
  `descartes` 컴퍼스 두 다리가 둘 다 원의 중심에서 나오고 경첩이 떠 있었다 ·
  `fermat` 접선이 할선보다 완만해 캡션과 반대되는 말을 하고 있었다(실제 −1.17, 그린 것 −0.42) ·
  `leibniz` 「확대도」 삼각형 비가 원본과 달랐고(0.646 vs 0.533) 캡션이 46px 잘려 있었다 ·
  `dedekind` `√2` 가 설명줄 위에 겹쳐 있었다. 전부 고쳤다.
- **검사기가 생겼다 — `python3 tools/verify_figures.py`.** 브라우저(Chromium)로
  글자 넘침·겹침을 잰다. 좌표만 읽는 검사는 **한글 폭을 40% 낮게 잡아** 잘린 캡션을
  통과시켰다. 브라우저가 없으면 「건너뜀」을 찍고 exit 0 이다.
- **기계가 못 잡는 것은 「그림이 옳은가」다.** 위 5건 중 셋이 그것이었다. 눈으로 볼 때
  물을 것 — 그린 기구가 실제로 그렇게 동작하나 · 그린 기울기가 본문 주장과 같은 방향인가 ·
  「확대도」가 원본과 같은 비율인가.
- **글꼴이 갈려 있다(손대지 않았다).** 11장 중 EB Garamond 계열 5장, Pretendard 계열 3장,
  섞인 것 3장. 한글은 어느 쪽이든 폴백이라 화면 차이는 크지 않지만 규약은 하나여야 한다.
  발행된 그림의 시각 변경이라 **`CLAUDE.md` 의 「큰 시각 변경 전 시안 확인」에 걸린다** —
  사용자 판단이 필요하다.
