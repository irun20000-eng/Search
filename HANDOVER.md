...
> **2026-09-04 — `갤러리URL` 을 146편에서 걷어내고 `person-wallis`(존 월리스)를 썼다.**
> 노트 10,355자 · 269줄 · 16절 · 시각화 7 · 출처 16. math **86편**, 17세기 인물 39편.
> 전날 생뱅상·버클리·레코드를, 그 전날 알콰리즈미·디오판토스·비에트를 썼다.

여기 중복해 적지 않는다. §11 의 **「새 권고」** 를 읽으면 된다. 요지만:

- **다음 집필 권고는 `person-cavalieri`(카발리에리, 17세기).** 부름이 월리스 노트로 **5편 → 6편**이
  됐고, `concept-integration` 발전단계의 앞자리(1635)이며, 이제 **인물 노트 둘**
  (`person-archimedes`·`person-wallis`)이 그를 앞사람으로 지목한다.
  **1635 카발리에리 → 1647 생뱅상 → 1656 월리스**로 구적법 삼각형이 닫힌다.
- **겹침 대상은 `concept-integration` §3**(「카발리에리의 불가분량」). 월리스 회차에서
  **착수 전에 그쪽을 정독하고 경계를 정한 것이 처음으로 잘 됐다** — 같은 순서를 지킬 것.
- **`person-lebesgue` 는 두 회차 연속 보류.** 부름 8편 84회로 최다이나 생몰·경력·당대 반응이
  `century-20c`·`concept-integration-advanced`·`episode-lebesgue-coins` 셋에 **이미 흩어져 있다.**
  쓸 거라면 축을 먼저 정할 것.
- **`갤러리URL` 은 폐지됐다**(146편에서 제거). 새 노트에 넣지 말 것 —
  `verify_math.py` 의 `audit_schema()` 가 `KNOWN_FIELDS ⊄ CARD_FIELDS` 를 **exit 1** 로 막는다.

**검수에서 반드시 볼 것 — 여섯.**

1. **각 `[n]` 을 그 문서의 출처 `제목` 과 눈으로 대조한다.** 특히 **한 원문을 여러 번호로 쪼갰을 때.**
   비에트·레코드·버클리·생뱅상·월리스 **다섯 회차 연속**으로 났다. **연보가 가장 위험하다** —
   압축돼 있어 한 줄이 관할을 넘기 쉽다(월리스에서 걸린 자리가 연보였다).
   ※ 이 지적은 내용을 깎지 않는다. 월리스에서는 **오히려 두껍게 만들었다** — 관할 밖 인용이
   드러낸 것이 「그 출처가 본문 절 없이 연보로만 쓰이고 있다」는 구조 갭이었고, 절을 세워 메웠다.
2. **`요약`·`태그`·`별칭` 도 본문과 같은 잣대로 본다.** 버클리·생뱅상 두 회차 연속이던 것이
   **월리스에서 끊겼다** — 착수 전에 넘김 선언을 먼저 정해 둔 것이 효과가 있었다.
3. **자매 노트와 모순되지 않는지 본다.** 인용할 때 그 노트를 **열어서** 확인할 것.
4. **한 노트 안에서 자기모순이 없는지 본다.** 절의 첫 문장이 유독 단정적이면 의심한다.
5. **지적 하나를 반영할 때 같은 유형을 문서 전체에서 grep 한다.**
6. **볼트 업로드 전 생성물을 통독한다.** 생뱅상·월리스 **두 회차 연속 0건** — 앞 단계가 잡고 있다.

- **볼트 사본은 올렸다.** Drive `002-수학사/03-인물`(`1Mf6JlxmonPut-6uXswjzZBQHDeMdFYxz`),
  **31,751바이트가 원본과 정확히 일치**(§2-5 의 표준 절차).
