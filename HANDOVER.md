...
> **2026-09-03 — 표 렌더 검사를 공용화하고 `person-berkeley`(조지 버클리)를 썼다.**
> 본문 7,531자 · 13절 · 시각화 5 · 출처 12. math **84편**, 18세기 31편.
> 같은 날 앞서 `person-recorde` 를, 전날 알콰리즈미·디오판토스·비에트를 썼다.

여기 중복해 적지 않는다. §11 의 **「새 권고」** 를 읽으면 된다. 요지만:

- **다음 집필 권고는 `person-saint-vincent`(그레고리우스 드 생뱅상, 17세기).**
  호명 2회(`concept-logarithm` 쌍)이고 **저장소에 아직 거의 안 적혀 있다** —
  버클리를 고른 것과 같은 근거다.
- **`person-lebesgue` 는 판단이 필요하다.** 부름이 **8편 84회로 최다**인데 이번에
  보류했다 — 생몰·경력·당대 반응이 `century-20c`·`concept-integration-advanced`·
  `episode-lebesgue-coins` 셋에 **이미 흩어져 있어** 인물 노트가 "모으는 일"이 된다.
  **쓸 거라면 축을 먼저 정할 것.**
- **부름의 두께가 큰 쪽이 항상 답은 아니다**(이번에 처음 쓴 기준). 이미 적힌 양이
  많으면 인물 노트가 설 자리는 **오히려 좁다.**
- **표 렌더 검사가 다섯 검증기에 걸렸다.** `tools/mdtables.py` 공용 + `tools/verify_tables.py`
  가 268파일을 훑는다. ⚠ **규칙이 둘이다** — `videos`·`guides` 는 이스케이프를 모르는
  자체 파서라 **셀 안에 `|` 를 쓸 방법이 아예 없다**(낱말로 풀어 적을 것).

**검수에서 반드시 볼 것 — 다섯으로 늘었다.**

1. **각 `[n]` 을 그 문서의 출처 `제목` 과 눈으로 대조한다.** 특히 **한 원문을 여러 번호로
   쪼갰을 때.** 비에트·레코드·버클리 **세 회차 연속**으로 났다.
2. **`요약`·`태그`·`별칭` 도 본문과 같은 잣대로 본다**(2026-09-03 신규). 버클리에서
   **본문은 규칙을 지키는데 `요약` 만 어긴** 자리가 나왔다. `요약` 은 갤러리 카드에
   그대로 나가므로 본문보다 눈에 먼저 띈다.
3. **자매 노트와 모순되지 않는지 본다**(2026-09-03 신규). 버클리가 [[레코드]]의 인과를
   **뒤집어** 적고 있었다. 다른 노트를 인용할 때는 그 노트를 **열어서** 확인할 것.
4. **지적 하나를 반영할 때 같은 유형을 문서 전체에서 grep 한다.**
5. **볼트 업로드 전 생성물을 통독한다.** 비에트 2건·레코드 1건·버클리 3건 —
   **세 회차 연속** 여기서 걸렸다.

- **볼트 사본은 올렸다.** `sync_math_obsidian.py --vault <임시>` 생성물을 그대로
  Drive `002-수학사/03-인물`(`1Mf6JlxmonPut-6uXswjzZBQHDeMdFYxz`)에 업로드했고,
  **바이트수 23,019 가 원본과 정확히 일치**하는 것으로 확인했다(§2-5 의 표준 절차).
