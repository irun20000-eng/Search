...
- `python3 tools/verify_figures.py` — 브라우저(Chromium)로 글자 **넘침·겹침**을 잰다.
...
**`fermat` 은 두 번 고쳤다 — 이게 이 회차의 교훈이다.** 첫 수정은 기울기만 맞췄는데, P 가
두 베지에의 이음매이자 **변곡점**이라 접선이 곡선을 X 자로 갈랐다(이탈 42px — 고치기 전
15.6px 보다 **나빠졌다**). 게이트는 통과했고 검수가 잡았다. 곡선을 「계속 완만해지는」
오목 곡선(로그)으로 다시 그려 접선이 위에 얹히게 했다(이탈 7.8px, 접선 −0.349 · 할선 −0.249).
**관계를 맞추는 것과 그림이 그렇게 보이는 것은 다르다.**
