...
> **2026-09-02 — `person-diophantus`(디오판토스)를 썼다.** 본문 9,497자 · 14절 · 시각화 7 ·
> 출처 15(국내 6 · 해외 9). math **81편**, **3세기 원자가 1편에서 2편**이 됐다.
> (같은 날 앞선 회차에 `person-al-khwarizmi` 를 써 80편이 됐다.)

여기 중복해 적지 않는다. §11 의 **「새 권고 — 다음 회차」** 를 읽으면 된다. 요지만:

- **선행 권고는 `관련세기` 부채 정리.** 이 필드가 `person-al-khwarizmi`·`century-bc3` 둘에
  남아 있는데 **`tools/` 도 `index.html` 도 읽지 않는다**(디오판토스 쪽은 이번에 지웠다).
  값도 의심스럽다 — 9세기 인물에 `[century-bc3]` 이 물려 있다. **스키마와 소비자를 만들든
  지우든 정할 것.** 새 문서가 아니라 30분짜리 정리다.
- **집필 권고는 `person-viete`(비에트, 16세기).** 호명 백로그 2회이자 `concept-algebra-notation`
  의 발전단계가 예약한 이름이다. 지도를 넓히지는 않지만 **디오판토스 → 비에트 → 데카르트**로
  기호법 계보의 가운데가 비어 있고, 이번 노트가 그 계보를 두 번 가리켰다.
- **`century-3c`·`century-9c` 는 둘 다 보류.** 원자는 각각 2편이 됐으나 **부름이 얇다.**
  세기 개관의 조건은 원자 수가 아니라 **부름의 두께**다(기원전 3세기는 원자 2편에 부름 30편).
- **고대·중세 인물은 「사료가 어긋나는 자리」 절을 기본 골격으로.** 유클리드·알콰리즈미에 이어
  세 번째다. 다만 **어긋남의 종류가 셋 다 달랐다** — 유클리드는 여러 전승이 다른 말을 해서,
  알콰리즈미는 국내 사전과 최근 학계가 갈려서, 디오판토스는 **전승이 하나뿐이라** 교차검증할
  상대가 없어서다. 셋째 경우엔 병기할 것이 없으므로 **"후대가 그렇게 적었다"로 귀속**한다.
- **검수에서 반드시 볼 것 둘.** (a) **각 `[n]` 을 그 문서의 출처 `제목` 필드와 대조한다** —
  이번에 다른 노트에서 문장을 옮기며 **그쪽 인용번호가 딸려 왔는데** 그 번호가 이쪽에도
  실재해 모든 자동 검사를 통과했다(절 하나의 근거가 통째로 비어 있었다).
  (b) **지적 하나를 반영할 때 같은 유형을 문서 전체에서 grep 한다** — "불확실 구간을
  최대값으로 단정" 이 검수가 든 두 자리 말고 **두 군데 더** 있었다(#193).
- `verify_math.py` 가 **미인용 출처를 경고한다**(정의만 되고 본문에서 안 불린 것). 믿고 쓸 것.
  다만 위 (a)는 못 잡는다 — **"있는 번호인가"만 보기 때문**이다.
- **볼트 사본은 올렸다.** `sync_math_obsidian.py --vault <임시>` 로 생성한 파일을 그대로
  Drive `002-수학사/03-인물`(`1Mf6JlxmonPut-6uXswjzZBQHDeMdFYxz`)에 업로드했고,
  **바이트수 25,549 가 원본과 정확히 일치**하는 것으로 확인했다(§2-5 의 표준 절차).
