...
> **2026-09-02 — `person-al-khwarizmi`(알콰리즈미)를 썼다.** 본문 7,409자 · 14절 · 시각화 6 ·
> 출처 12(국내 6 · 해외 6). math **80편**, **9세기 원자가 0편에서 2편**(개념+인물)이 됐다.

여기 중복해 적지 않는다. §11 의 **「2026-09-03 에 이어서 할 것」** 을 읽으면 된다. 요지만:

- **권고는 `person-diophantus`(디오판토스, 3세기).** 3세기는 원자 1편뿐인데 **그를 부르는
  노트가 4편**이고, 그중 `person-fermat` 이 부르는 것은 **『산술』 여백**이라는 구체적 사건이다.
- **이번 회차가 방법을 하나 보정했다** — 원자가 서는 것과 **세기가 설 수 있는 것은 다르다.**
  9세기는 원자 2편이 됐지만 **부름이 2편뿐**이라 `century-9c` 는 아직 이르다(기원전 3세기는
  원자 2편에 부름 30편이었다). 세기 개관은 **부름의 두께**가 조건이다.
- **고대·중세 인물은 「사료가 어긋나는 자리」 절을 기본 골격으로.** 알콰리즈미에서 그 예고가
  그대로 맞았다 — 「지혜의 집」을 두고 국내 사전(830년 설립 학술원)과 최근 학계(구타스: '창고'의
  오역, 당대 언급 희박)가 정면으로 갈렸고, 생몰·출신 표기도 자료마다 달랐다.
  **국내 자료만 보고 쓰면 틀린 확신을 갖게 되는 자리다.**
- `verify_math.py` 가 **미인용 출처를 경고한다**(정의만 되고 본문에서 안 불린 것). 이번에
  2건을 그 경고로 잡았다 — 세는 게이트가 못 잡는 유형인데 이 검사기는 잡으니 믿고 쓸 것.
