...
### 2-6. 볼트 사본 3편이 리포보다 한 판 뒤에 있다 — 작음, 판단 필요

2026-08-31 에 **이미 볼트에 있는** 세 편의 본문을 리포에서 고쳤다.
`person-euler`(§5 신설 + 절 번호 밀기) · `person-laplace`(무인용 문장 교체) ·
`reports/infinity-set-theory`(§6 인과 암시 교정).

**동기화가 덮지 않는다.** `sync_obsidian.py`·`sync_math_obsidian.py` 는 **없을 때만 쓰고
이미 있으면 건드리지 않는다** — 토요일 위키링크 작업을 지키려는 설계다. 그래서 볼트 사본은
그 세 편만 한 판 뒤에 있다.
...
2. 볼트에서 그 세 편만 손으로 갈아 끼운다(각각 한 절 남짓).
3. 세 편만 지우고 동기화를 돌린다 — 그 파일에 붙여 둔 볼트 메모가 있으면 함께 사라진다.
