...
> **2026-09-03 — `person-recorde`(로버트 레코드)를 썼다.** 본문 7,971자 · 12절 · 시각화 5 ·
> 출처 14(국내 5 · 해외 9). math **83편**, 16세기 10편.
> 전날 알콰리즈미·디오판토스·비에트 3편 + `관련세기` 부채 정리(#195)를 했다.

여기 중복해 적지 않는다. §11 의 **「새 권고」** 를 읽으면 된다. 요지만:

- **선행 권고는 `reports/`·`guides/` 표 렌더 검사.** 이번에 만든 `table_defects()` 가
  **math/ 에만 걸린다.** 실측에서 `reports/derivative-rate-of-change-advanced` 의
  `|∇f|` 2건이 **이미 발행된 채** 나왔다 — 절댓값·노름 기호를 쓰는 글이 거기 더 많다.
  검사를 공용으로 빼거나 `reports/` 용 검증기를 만들 것. 새 문서가 아니라 도구 일이다.
- **기호법 계보가 닫혔다.** `concept-algebra-notation` 의 발전단계 예약이 **전부 해소**됐다 —
  디오판토스 → **레코드** → **비에트** → 데카르트가 다 섰다. **다음 집필 대상은
  `verify_math.py --backlog` 이 정한다.**
- **`century-3c`·`century-9c` 는 여전히 보류.** 원자는 섰으나 **부름이 얇다.**
  세기 개관의 조건은 원자 수가 아니라 **부름의 두께**다.

**검수에서 반드시 볼 것 — 넷으로 늘었다.**

1. **각 `[n]` 을 그 문서의 출처 `제목` 필드와 눈으로 대조한다.** 검사기는 "있는 번호인가"만
   본다. 특히 **한 원문(MacTutor 한 페이지)을 여러 번호로 쪼갰을 때** 본문이 경계를
   넘나든다 — 비에트·레코드 두 회차 연속으로 났다.
2. **지적 하나를 반영할 때 같은 유형을 문서 전체에서 grep 한다.** 비에트 2자리(#199),
   레코드 1자리(#202)가 더 나왔다.
3. **볼트 업로드 전 생성물을 통독한다.** 위 둘이 다 여기서 걸렸다. 읽는 자리가 달라지면
   다른 것이 보인다.
4. **조사 결과가 코퍼스 대부분을 가리키면 검출기를 먼저 본다.** 표 결함 전수 조사 첫 판이
   3,953건(=모든 표 행)을 뱉었고, 기준을 고치니 4건이었다. `KNOWN_FIELDS` 첫 판이 12편을
   오탐한 것과 **같은 유형이 이틀 만에 재발**했다.

- **볼트 사본은 올렸다.** `sync_math_obsidian.py --vault <임시>` 생성물을 그대로
  Drive `002-수학사/03-인물`(`1Mf6JlxmonPut-6uXswjzZBQHDeMdFYxz`)에 업로드했고,
  **바이트수 25,209 가 원본과 정확히 일치**하는 것으로 확인했다(§2-5 의 표준 절차).
