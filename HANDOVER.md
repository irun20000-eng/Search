...
- **대상 고르는 기준.** 「노트가 **이미 출처를 달고 말로 적어 둔 것** 중 그림이 대신 말할 수 있는 것」.
  ...
  수집 회차가 딸려 온다.
- **그림은 반드시 래스터라이즈해서 본다.** 이 러너에서 된다 —
  `pip install cairosvg` + `apt-get install -y fonts-nanum`(한글 글리프는 네모로 나오지만
  **자리와 폭은 정확**하다). 좌표 경계 검사만으로는 **겹침·덩어리짐을 못 잡는다**(LESSONS 2026-09-08).
- **아직 눈으로 안 본 것 8장.** 이번에 만든 3장만 렌더해서 봤다. **기존 8장은 소스도 렌더도
  이 회차에서 검사하지 않았다** — 같은 유형(라벨이 도형을 파고들기·캡션이 눈금과 겹치기)이
  있을 수 있다. 다음 회차에 전수로 볼 것.
- **볼트는 이번 회차만큼 뒤처진다.** 세 노트 모두 볼트에 **이미 있으므로**
  「없을 때만 쓴다」 규칙상 `sync_math_obsidian.py` 가 건드리지 않는다 — 볼트 사본에는
  새 그림이 없다. 이것이 §2-5 가 말하는 그 뒤처짐이고, `--check` 로 잰다.
