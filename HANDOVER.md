...
> **2026-09-04 — `person-lebesgue`(앙리 르베그)를 썼다.** 본문 9,705자 · 246줄 · 16절 · 시각화 8 · 출처 18.
> math **88편**, 20세기 21편. **네 회차 연속 보류였던 이름**이고, 보류의 이유였던 「축」을
> **겹침 판정을 회차의 첫 단계로 세워** 정했다 — 세 노트(29·16·13회)가 방법과 당대의 반대를 이미
> 다 갖고 있었고, **어디도 쓰지 않던 MacTutor 의 한 문단**(일반화를 해냈는데 일반화를 두려워했다)이
> 축이 됐다. 볼트 `002-수학사/03-인물`, **32,915바이트 일치**.
>
> **새 게이트가 만든 지 한 회차 만에 커밋을 막았다** — `audit_contrib()` 가 `기여인물` 누락을 잡았다.
> 손으로 챙기던 부수작업이 기계가 막는 일이 됐다.
>
> **2026-09-04 — 선행 작업도 닫았다: `기여개념` ↔ `기여인물` 단방향 21건.**
> 하나씩 판정해 **넣은 것 14 · 뺀 것 7**, 단방향 총계 **88 → 67건**.
> 재발은 `verify_math.py` 의 `audit_contrib()` 가 막는다(양방향 시험 완료).
> **검수가 내 추가분 3건을 잡았다** — 기준을 뺄 때는 엄히, 넣을 때는 느슨하게 썼다.
> 라그랑주→미분법은 양쪽에서 뺐고, 카르다노·라플라스는 `concept-probability` 본문에 자리를 만들었다
> (§1 「한 세기 앞서 같은 것을 센 사람」 신설 · 출처 13 신설 · 13,935자).
>
> 같은 날 앞서 `person-cavalieri`(10,039자 · 258줄 · 17절 · 시각화 7 · 출처 21)와 월리스를 쓰고
> `갤러리URL` 을 146편에서 걷어냈다. math **87편**, 17세기 인물 40편.
> **1635 카발리에리 → 1647 생뱅상 → 1656 월리스**로 구적법 삼각형이 닫혔다.

여기 중복해 적지 않는다. §11 의 **「새 권고」** 를 읽으면 된다. 요지만:

- ~~다음은 집필이 아니라 선행이다 — `기여개념` ↔ `기여인물` 단방향 21건~~ → **닫았다**(위 상자).
- ~~집필 후보는 `person-lebesgue` 가 여전히 최다 부름이나 세 회차 연속 보류~~ → **썼다**(위 상자).
  **호명 백로그가 평평해졌다** — 30종이 전부 1~2회다. 다음 대상은 부름의 두께로는 정해지지 않으므로
  `math/ROADMAP.md` §11 「새 권고」 표의 남은 줄(도해 부채 · `century-3c`/`century-9c` 보류)을 볼 것.
- **겹침 판정을 회차의 첫 단계로 세우는 것이 값을 했다.** 월리스·카발리에리에서 「착수 전에 가른다」로
  배운 것을 르베그에서는 **판정 자체가 축을 만드는 데까지** 밀었다. 부름이 두꺼운 이름일수록
  「모으는 일」이 되기 쉬우니, **세 노트가 이미 가진 것을 실측한 뒤 남은 자리를 찾는다.**
- **`기여인물` 에 이름을 넣기 전에 `grep -c '<인물명>' <개념노트>` 를 돌린다.** 0 이면 선택지는 둘 —
  **넣지 않거나, 본문에 그 사람의 자리를 만들거나.** 이번에 셋 중 둘은 후자가 맞았다
  (출처가 이미 그 이름을 담고 있었고 본문만 빠뜨린 상태였다).
- **겹침은 착수 전에 가른다.** 월리스·카발리에리·르베그 세 회차 연속으로 효과를 봤다 —
  겹치는 노트의 해당 절을 **먼저 정독하고** 무엇을 그쪽에 남길지 정한 뒤 쓰기 시작한다.
  르베그에서는 이 판정이 **축을 만드는 데까지** 갔다(위 항목).
...
### 2-8. 개념노트 서가(`concept/`) — 2026-09-05 회차 메모

> **`sample-mean-variance`(표본평균의 평균과 분산)를 썼다.** 8,978자 · 293줄 · 16절 · 시각화 65 ·
> 국내 3 · 해외 3. 개념 한 장까지 그려 **9편 9장**이 됐다(빈 자리 없음).
> 볼트 `003-카드뉴스학습자료`, **23,864바이트 일치**.

이 서가를 다시 만질 때 알아 둘 것 넷.

- **요청이 「가이드」로 와도 게이트를 먼저 읽는다.** `guides/` 는 **실습 워크스루 전용**이다 —
  단계마다 「화면에서 확인할 것」이 정확히 1개, `사전 준비물` 필수, **검증된 유튜브 3편 이상**.
  수학·개념 학습자료는 `concept/`(TL;DR → 핵심 포인트 → 번호 절 → 종이 위 검산 → 오해)가 맞는다.
- **개념 한 장에 결합 문자를 쓰지 말 것.** `X̄` 가 러너 렌더에서 **사라졌다** —
  `E(X̄)=μ` 가 `E(X)=μ` 로 그려졌는데 스키마·넘침·여백 검사는 **전부 통과**했다.
  인라인 overline(`<span style="text-decoration:overline">X</span>`)으로 그릴 것.
  **노트 본문(마크다운)은 그대로 둬도 된다** — 갤러리·볼트는 웹폰트라 결합 문자가 살아 있다.
- **PNG 는 반드시 눈으로 본다.** 이번 한 장은 세 번 고쳤고 **둘은 그림을 봐야만 보이는 것**이었다
  (글자 소실 · 카드 1 만 비어 보임). 러너 로그가 잡은 것은 넘침 하나뿐이다.
- **빌더는 `--all` 을 try/except 없이 돈다.** 한 슬러그가 넘치면 `SystemExit` 으로 **전체가 멈춘다**.
...
| 2026-09-17 | `napier-two-moving-points` · `weierstrass-zoom-never-smooth`(개념·인물 둘에 물림) | 인물 2 · 개념 1 |

**열려 있는 판단 — 도해 생성 스크립트를 리포에 넣을 것인가** (2026-09-20 에 보류)

도해는 좌표를 손으로 찍지 않고 파이썬으로 만든다. 그 스크립트가 **지금은 스크래치패드에만
있어 세션과 함께 사라진다.** 2026-09-20 에 「화면 y 가 아래로 커진다」를 코드 주석에 적어
놓고 검수에게 「그 기록은 다음 사람에게 안 닿는다」고 지적받았다 — 그래서 그 교훈은
`LESSONS.md` 체크리스트로 옮겼고, **기록의 소실은 그것으로 막혔다.**

남은 물음은 별개다: **스크립트 자체를 서가에 둘 것인가.** 지금 `math/` 에는 도해를 만드는
스크립트가 한 편도 없어 관례를 새로 세우는 일이고(경로·재현성·게이트 편입 범위를 함께
정해야 한다), 한 회차에 끼워 결정할 크기가 아니다. **세션이 임의로 정하지 말 것.**
...
⚠ **숫자는 끝내 재지 못했다.** 「`lining-nums` 를 갖고 있으니 괜찮다」고 적었다가 검수가
뒤집었다 — 그 선언은 **폰트가 `lnum` 피처를 가질 때만** 듣는데, 선언을 지운 사본과 원본의
렌더가 **픽셀 단위로 같았다**(둘 다 같은 해시). 구글폰트 EB Garamond 에도 `lnum`·`onum` 이
없다. 게다가 두 측정 환경(컨테이너·러너) 어디에도 스택 앞머리가 없어 실제로 잡히는 것은
NanumMyeongjo / Noto Serif CJK 이고 **둘 다 기본이 라이닝**이다 — 즉 **사고가 났던 Georgia 는
한 번도 안 재졌다.** 숫자를 지키는 것은 선언이 아니라 **산세리프 `<tspan>`** 이다
(`vibrating-string-initial-shape` 의 `u(x, 0)` 이 그 예이고, 이번 통일에서 한 번 삼켰다가
검수가 잡아 되돌렸다). 재발은 이제 `verify_figures.py` 의 `check_fonts` 가 막는다 —
예외는 **이유와 함께** `FONT_EXCEPTIONS` 에 적는다.
...
남은 상위: `person-saint-vincent` 29(단 쌍곡선은 이미 그렸다) · `person-gregory` 23 ·
`person-wallis` 14 · `person-pascal` 12 · `person-gauss` 10(정십칠각형 작도).

**대상 고르는 기준.** 「노트가 **이미 출처를 달고 말로 적어 둔 것** 중 그림이 대신 말할 수
있는 것」. 새 사실을 들이지 않으므로 도해가 인용을 새로 요구하지 않는다. 이 기준을 벗어나면
그림 하나에 수집 회차가 딸려 온다.

**어떻게 재나 — 글자는 기계가, 그림은 사람이.**

- `python3 tools/verify_figures.py` — 브라우저(Chromium)로 글자 **넘침·겹침·관통·가려짐**을 잰다.
  관통(= 잉크가 글자 상자 안쪽을 지남)은 만들면서 **다섯 번 틀렸고 다섯 번 다 왕복으로 드러났다** —
  ① `line`·`polyline` 만 보면 리포 잉크의 절반을 놓친다(`dice-sum-grid` 는 전부 `<rect>`,
  `descartes` 컴퍼스는 `<path>`) → 기하 요소를 `getTotalLength()` 로 훑는다.
  ② 뒤에 그린 **불투명** 도형에 가린 선은 화면에 없다 → `elementsFromPoint` 로 거른다.
  ③ 글자는 상자 전체가 히트 테스트에 걸린다 → 스택에서 걷어내야 진짜를 잡는다.
  ④ **점선은 틈에서 아무것도 안 걸린다** → 잴 때만 `stroke-dasharray` 를 끈다(위상 운에 좌우됐다).
  ⑤ **반투명은 가리지 못한다** → 실효 알파 0.5 미만은 차단으로 안 센다.
  ⑥ 굵은 선은 중심선이 상자 밖이어도 잉크가 들어온다 → 반폭만큼 넓혀 잰다.
  ⑦ `fill` 을 읽었으면 `fill-opacity` 를 곱해야 한다(`stroke-opacity` 를 곱하고 있었다).
  ⑧ 거울상인 **가려짐**(불투명 도형이 글자를 덮음)은 **그 점에 글자가 실제로 그려져 있을 때만**
  판정해야 한다 — 안 그러면 배경 rect 가 맨 앞으로 잡혀 11장 124건 오탐이 난다.
  그리고 **옅은 격자선(지면 대비 명암비 1.30 미만)은 세지 않는다** — 라벨 밑 격자는 정상 배치다.
  1.30 은 격자 #EDE4D4(1.24)만 빼고 구조선 #E2DACB(1.37)·#DED3C0(1.46)은 감시 안에 남긴 값이다.
- **일화는 자수 상한 2,500자다** — 유형마다 하한만 있는 게 아니다(「짧아야 재사용된다」).
  `episode-bernoulli-tomb` 이 그 턱밑에 있다 — `python3 tools/verify_math.py episode-bernoulli-tomb` 이
  지금 자수를 찍어 준다(2,500 에서 빼면 남은 여유다). **여기 수를 적어 두지 않는다**: 지난번에 적었더니
  캡션이 자란 뒤 「여유 34자」가 실제 24자를 42% 부풀린 채 남았다. 캡션 한 줄이 34자쯤이니
  **이 노트에 문장을 더 넣으려면 다른 곳을 줄여야 한다.**
- **게이트가 FAIL 해서 고쳤으면 빌더를 다시 돌릴 것 → 이제 게이트다: `python3 tools/verify_builders.py`.**
  2026-09-09 에 안 돌려서 `manifest.json` 에 **실패 시점의 자수(2,547)가 굳었고** 그 값이 갤러리 카드에
  나갔다. ROADMAP 「자동 측정」도 없는 실패를 기록했다. **재는 것은 멱등성이다** — 산출물이 지금 소스의
  고정점인가. 빌더를 돌려 결과가 바뀌면 낡은 것이다. **`git diff` 로 재면 안 된다**(HEAD 와 비교하므로
  ...
  **보고서 제목을 고치고 manifest 를 안 고쳐도 게이트가 통과한다**(실측 확인). 그 서가의 진짜 낡음을
  ...
  `title` 은 16편이 카드용으로 손질돼 있고 `tldr` 은 72편 전부가 손으로 쓴 문장이라 규칙 하나로 다시 지으면
  ...
  **2026-09-09 2차 — 검수가 내 분류를 뒤집었다.** `sources` 를 「소스가 없다」로 묶었는데 그것은
  **manifest 쪽 표기 불일치(int/dict)**였을 뿐이고 값은 `소스수` 에서 그대로 나온다.
  **2026-09-09 3차 — 그 대조도 반쪽이었다(검수가 또 잡았다).** `소스수` 와 manifest 는 **서로에게서
  베낀 사본**이라 **함께 틀리면 초록**이다 — 실제 `## 출처` 목록을 세니 2편이 그 상태로 통과 중이었다
  (`chrome-extensions-work` 13≠14 · `jacobian-advanced` 16≠15, 후자는 네이버 블로그 `[J6]` 가
  `**해외**` 구획에 있었다). 이제 **목록 자체**와 `tldr` 이 적은 수까지 넷을 함께 잠근다.
  **형태 통일(사람 몫)과 값 대조(기계 몫)를 하나로 묶어 값 대조까지 포기했던 것**이다.
  대조를 넣자 어긋남 5편이 나왔고 **넷은 `report.md` 의 `소스수` 가 거짓**이었다(정본 쪽이 틀린 자리).
  실제 출처 목록을 세어 고쳤다. `title` 도 손질 16편을 허용목록으로 고정해 **나머지 56편을 잠갔다**.

  **3차 재검수가 다시 두 구멍을 짚어 같은 PR 에서 닫았다** — ① 라벨 없는 `- 매체 「제목」 — URL`
  줄은 세 사본 어디에도 안 세어졌다(`hermes-agent-quick` 이 목록 8인데 셋 다 6이었다. 두 줄에
  `[7][8]` 을 붙이고 국내 3→5). ② `tldr` 의 `(국내a·해외b)` 를 캡처해 놓고 총계만 비교해
  **내역이 뒤바뀌어도 통과**했다.

  ⚠ **남은 것 하나는 데이터가 아니라 내용이라 손대지 않았다.** `ai-engineering-evolution-graph`
  의 국내 출처 2건이 「국내 IT 매체·블로그의 … 소개(다수)」처럼 **매체명도 URL 도 없는
  뭉뚱그림**이다. 새 게이트는 이것을 정식 출처 2건으로 세므로 국내 2가 그대로 정당화된다.
  CLAUDE.md 무환각(「출처 없는 단정 0건」)에 걸리는 자리 — **실제 매체·URL 을 달거나 목록에서
  내려야 하고, 그건 발행된 본문을 고치는 일이라 다음 회차에 사람이 판단할 것.**
  ⚠ **여전히 아무 게이트도 안 잡는 것 — 목록을 여기 베끼지 않는다.**
  `python3 tools/verify_reports_meta.py` 가 돌 때마다 그 목록을 직접 찍고, `--help` 로도 나온다.
  **이 한계 목록을 문장으로 네 벌 유지하다가 이 PR 에서만 세 번 낡았다**(2차 때 `verify_builders.py:44`,
  3차 때 이 줄). 「손으로 적은 숫자는 빌더가 안 따라온다」와 같은 이유이고 처방도 같다 —
  **문장 대신 측정 명령을 적는다.** 그때그때 짧게 알아야 하면 `CLAUDE.md` 명령 블록과
  CI 요약에 요약본이 있다(그 둘은 사람이 실제로 보는 자리라 남겨 둔다).
  그중 `sources` **표기 형태** 통일은 발행물 표시가 바뀌는 일이라 **사람이 정할 자리**다 —
  세션이 임의로 밀어붙이지 말 것.
  ...
  **세 번째 시계 누출 경로가 지금은 막혀 있다(2026-09-09 검수).** `build_link_index.py:160` 은 math 노트에
  ...
  지금 도달 불가인 이유는 `verify_math.py` 의 `COMMON_REQUIRED` 가 `날짜` 를 강제하기 때문이다
  (실측: 88편 중 `날짜` 없는 노트 0편). **게이트가 게이트를 지탱하는 구조**이니 `날짜` 필수를 뺄 때 여기를 함께 볼 것.
  **다음 회차 후보 하나 더** — `math/manifest.json` 의 `generated` 는 읽는 화면 0곳이고 코드 소비자도
  위 폴백 하나뿐인데 그마저 도달 불가다. **아무도 안 읽고, 어떤 재빌드도 재현하지 않으며, 게이트가 영구 예외를
  하나 유지해 줘야만 하는 값**이다. 걷어내거나 소스 유래 값(노트 `날짜` 의 최댓값)으로 바꾸면 `CLOCK` 한 자리와
  위 누출 경로가 함께 사라진다. 「죽은 필드」 판정을 두 번 해 본 자리와 같은 유형이다(2026-09-03·09-04).
...
- **검사기의 여유폭 둘 — 다음에 라벨을 놓을 때 알고 있을 것(2026-09-09 검수 실측).**
  ...
  판정한 자리다). +3px 부터 뜨는 것들은 「축 옆 y」·「분수선 옆 dx/dy」처럼 정상이므로 현 확장폭이
  과하지는 않다. **`Q` 만큼 빡빡하게 라벨을 놓으면 FAIL 이 뜬다.**
  ② **명암비 문턱과 워시 사이 여유가 0.02~0.06 이다.** `#9C5A2C` 워시의 합성 명암비는
  0.16 → 1.24(hyperbola 음영) · 0.18 → 1.28 이고 문턱은 1.30 이다. **워시를 0.20 으로 올리면
  음영 윤곽이 관통으로 잡히기 시작한다.** 실패 방향이 오탐이라 위험하진 않지만 알고 만질 것.
  좌표만 읽으면 한글 폭을 모른다(어림 0.62×font-size 는 실측 0.99 대비 38% 짧아 46px 잘린
  캡션을 통과시켰다). 브라우저를 못 찾으면 **exit 2**(`--allow-skip` 이면 0) — 「검사 안 함」과
  「통과」를 같은 값으로 내보내지 않는다. 경로는 `chromium-*` 로 훑으므로 러너 이미지가
  올라가도 조용히 건너뛰지 않는다.
- ⚠ **cairosvg 는 판정에 쓰지 말 것.** `text-anchor="middle"` + `<tspan>` 에서 조각마다 따로
  가운데 정렬해 **없는 겹침을 만들어 낸다**(멀쩡한 `u(x, 0) = a·sin(πx/L)` 이 겹쳐 보였다).
- **「그림이 옳은가」는 기계가 못 잰다.** 이번 6건 중 **넷**이 그것이었다. 물을 것 —
  ① 그린 기구가 실제로 그렇게 동작하나 ② 그린 기울기가 본문 주장과 같은 방향인가
  ③ 「확대도」가 원본과 같은 비율인가 ④ 접선이 정말 접해 보이나 ⑤ 덧그린 상자가 곡선을 가리지 않나.

**기존 8장에서 나온 결함 6건 (전부 수정).**

| 도해 | 무엇 | 종류 |
|---|---|---|
| `descartes` | 컴퍼스 두 다리가 **둘 다 원의 중심에서** 나오고 경첩이 떠 있었다 | 그림 |
| `fermat` | **접선이 할선보다 완만**했다(실제 −1.17, 그린 것 −0.42) — 캡션과 반대 | 그림 |
| `leibniz` | 「확대도」 삼각형 비가 원본과 달랐다(0.646 vs 0.533) | 그림 |
| `leibniz` | 확대 상자가 **곡선을 21px 덮고** 있었다(x 279~300) — **검수가 찾았다** | 그림 |
| `leibniz` | 하단 캡션이 지면 밖으로 **46px 잘렸다** | 글자 |
| `dedekind` | `√2` 가 ③ 설명줄 위에 **겹쳐** 있었다 | 글자 |

**`fermat` 은 세 번 고쳤고, 가운데 두 번은 맞던 것을 부쉈다 — 이게 이 회차의 교훈이다.**

| 판 | 접선 | 할선 | 곡선과 접하나 | 본문(2x+e > 2x)과 |
|---|---|---|---|---|
| 원본 | −0.4205 | −0.5458 | ✗ | **✔** |
| 1차 | −1.1664 | −0.5458 | ✗ (변곡점, 이탈 42px) | ✗ |
| 2차 | −0.3487 | −0.2486 | ✔ (7.8px) | ✗ |
| 3차(지금) | −0.3486 | −0.4695 | ✔ (5.1px) | ✔ |

원본은 접선 기울기가 곡선과 안 맞았다. 나는 **그것만** 보고 고쳤고, 1차는 변곡점 때문에
접선이 곡선을 X 자로 갈랐으며(원본보다 나빠졌다), 2차는 곡선을 오목(로그)으로 바꿔
접선은 얹었지만 **오목함수는 언제나 할선이 접선보다 완만하다** — 그림 **바로 위 본문**이
`(x+e)² − x² → 2x+e → 2x` 를 계산하는데 그 반대를 보이게 됐다. 3차에서 본문이 다루는
`x²` 그대로 **볼록** 포물선으로 그려 둘 다 만족했다. **고치기 전에 「이 그림이 지금
맞히고 있는 것」을 적지 않은 것이 원인이다.** 게이트는 네 판 모두 통과했다.

**손대지 않은 것 — 글꼴이 갈려 있다.** 실측 **EB Garamond 전용 6장**(cavalieri · descartes ·
dice-sum-grid · fermat · hyperbola · leibniz) · **Pretendard 전용 2장**(least-squares ·
riemann) · **섞인 것 3장**(binomial · dedekind · vibrating-string). 규약은 하나여야 하지만
**발행된 그림의 시각 변경**이라 `CLAUDE.md` 의 「큰 시각 변경 전 시안 확인」에 걸린다 —
사용자 판단이 필요하다.

**볼트는 이 회차만큼 뒤처진다.** 그림이 들어간 노트가 볼트에 **이미 있으므로**
「없을 때만 쓴다」 규칙상 `sync_math_obsidian.py` 가 건드리지 않는다. `--check` 로 잰다(§2-5).

**열린 것 — 『구수략』의 네이피어 주산 (2026-09-24, PR #298 검수에서).** `century-17c` §11 이
「두 전통이 서로를 몰랐다 · 접촉은 훨씬 뒤에야」라고 적고 있었는데, 최석정의 『구수략』이
**네이피어의 계산 막대(籌算)를 소개한다**는 서술이 검색 요약 둘(KAIST 수리과학과 소식지 「최석정」 ·
한국민족문화대백과 「구수략」 — 노트의 출처 [16] 과 같은 항목)에 나온다. 원문은 **클라우드에서
열리지 않았다**(두 도메인 모두 egress 차단). 그래서 #298 은 **틀린 단정만 뺐고**(「두 전통을 굴린
동력이 달랐다」로) 새 사실은 넣지 않았다. **할 일: 원문 두 곳을 열어 확인한 뒤 §11 에 출처와 함께
「한 방향의, 책을 거친 접촉」을 넣는다.** 그림(`century-17c-two-generations.svg`)의 ⚠5 는 이미
「접촉은 이 그림이 다루지 않는다」로 중립이라 다시 그릴 필요는 없다.
같은 절의 「서로 다른 도구로 풀고 있었다」도 인용이 없다 — 주산이 네이피어 막대라면 도구가 한 방향으로 건너갔으니 그때 함께 다듬을 것.

**열린 것 — 19세기 「바이어슈트라스 함수는 [8]이 1874로 적는다」의 근거 (2026-09-24, PR #299 검수에서).**
`century-19c` 본문이 이 함수의 해를 [5] 1872 / [8] 1874 로 갈라 적는데, [8](수학백과 「해석학」)의
frontmatter 제목에는 1874 도 바이어슈트라스도 없고 검색 요약으로도 확인되지 않았다. 도해
...
`python3 tools/verify_math.py --symmetry` 가 보고한다. **전체는 여전히 게이트가 아니다.**
다수가 이해편/확장편 쌍의 기존 관례라 **설계상 정상일 수 있다** — 줄이기 전에 관례부터 정할 것.

**예외 하나**: `기여개념 → 기여인물` 방향은 **0건이고 `audit_contrib()` 가 지킨다.**
`기여인물` 이 갤러리 검색 인덱스라 한쪽만 적히면 그 이름으로 개념을 못 찾기 때문이다.
반대 방향(`기여인물 → 기여개념`) 10건은 검색에 영향이 없어 보고로 남았고,
그중 세기 노트 2건은 **스키마상 대칭이 불가능**하다.
