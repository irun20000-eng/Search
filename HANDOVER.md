...
> **2026-09-02 — `person-viete`(프랑수아 비에트)를 썼다.** 본문 9,174자 · 14절 · 시각화 7 ·
> 출처 16(국내 5 · 해외 11). math **82편**, 16세기가 9편이 됐다.
> 같은 날 앞서 `person-al-khwarizmi`·`person-diophantus` 를 썼고 **죽은 frontmatter 필드 정리**(#195)도 했다.

여기 중복해 적지 않는다. §11 의 **「새 권고 — 다음 회차」** 를 읽으면 된다. 요지만:

- **다음 권고는 `person-recorde`(레코드, 16세기).** `concept-algebra-notation` 의 발전단계가
  적어 둔 이름이고 **1557년 등호**라는 단일 사건이 뚜렷하다. `century-16c` 가 이미
  "레코드가 1557년에 등호를"이라 적어 두어 갚을 자리가 서 있다. 비에트보다 가볍다.
- **기호법 계보의 가운데가 채워졌다** — 디오판토스(3세기) → **비에트(1591)** → 데카르트(1637).
  세 인물 노트가 다 서 있고 판정은 `concept-algebra-notation` 쌍이 갖는다.
- **`century-3c`·`century-9c` 는 보류.** 원자는 각각 2편이 됐으나 **부름이 얇다.**
  세기 개관의 조건은 원자 수가 아니라 **부름의 두께**다(기원전 3세기는 원자 2편에 부름 30편).
- **`관련세기` 부채는 닫혔다**(#195). 전수 조사에서 `위키링크` 가 하나 더 나와 둘 다 지웠고,
  `verify_math.py` 의 **`KNOWN_FIELDS` 가 스키마 밖 필드를 거부**한다. 새 필드를 넣으려면
  `KNOWN_FIELDS`·`CARD_FIELDS`·**실제로 읽는 코드** 셋을 함께 만들 것.

**검수에서 반드시 볼 것 — 세 회차 연속 같은 자리에서 났다.**

1. **각 `[n]` 을 그 문서의 출처 `제목` 필드와 눈으로 대조한다.** 검사기는 "있는 번호인가"만
   본다. 디오판토스에서는 *다른 노트의* 번호가 딸려 왔고, 비에트에서는 **같은 원문(MacTutor
   한 페이지)을 다섯 번호로 쪼개 놓고 본문이 경계를 넘나들었다.** 둘 다 모든 자동 검사를 통과했다.
2. **지적 하나를 반영할 때 같은 유형을 문서 전체에서 grep 한다.** 디오판토스에서 두 자리(#193),
   비에트에서 두 자리(#199)가 더 나왔다. 검수는 대표 사례를 들지 전수를 세지 않는다.
3. **볼트 업로드 전 생성물을 통독한다.** 비에트의 남은 둘은 게이트·검수·기계 대조를 다 통과한
   뒤 **볼트 생성물을 읽다** 발견했다. 읽는 자리가 달라지면 다른 것이 보인다.

- **볼트 사본은 올렸다.** `sync_math_obsidian.py --vault <임시>` 생성물을 그대로
  Drive `002-수학사/03-인물`(`1Mf6JlxmonPut-6uXswjzZBQHDeMdFYxz`)에 업로드했고,
  **바이트수 28,402 가 원본과 정확히 일치**하는 것으로 확인했다(§2-5 의 표준 절차).
