...
> **2026-09-01 — `century-bc3`(기원전 3세기)를 썼다.** 본문 8,612자 · 17절 · 시각화 6 ·
> 출처 18(국내 10 · 해외 8). math 79편이 됐고 **개관 없는 세기가 넷에서 셋(3·9·14)으로 줄었다.**

여기 중복해 적지 않는다. §11 의 **「2026-09-02 에 이어서 할 것」** 을 읽으면 된다. 요지만:

- **권고는 `person-al-khwarizmi`(알콰리즈미, 9세기)** — 세기가 아니라 **인물부터**다.
  개관 없는 세기 셋(3·9·14)은 전부 개념 1~2편뿐이라 개관을 지탱할 인물이 없다.
  이번 회차가 그 방법을 검증했다: **인물이 서면 세기가 선다**(-3세기 = 원자 2편 + 부름 30편 → 개관).
- **왜 9세기인가** — 빈 세기가 **4~8세기 5개 · 10~13세기 4개**로 가장 긴 구간인데 그 한복판이
  이슬람 수학이고, 지금 서가에 9세기 원자가 **0편**이다. 한 편이 아홉 세기로 가는 관문이 된다.
  게다가 `century-bc3` §9 가 "그 사이를 이슬람 세계와 중세 유럽의 필사가 메운다"로 끝나
  이미 그 자리를 약속해 뒀다(§0 유형②).
- 차선은 `person-diophantus`(3세기). 호명 2회 4종(비에트·버클리·르베그·그레고리우스 드
  생뱅상)은 **전부 이미 개관이 있는 세기**라 지도를 넓히지 않는다.
- **세기 개관의 비용이 실측으로 확인됐다** — 게이트 7,673자에 실제 8,612자. 인물 노트 한 편의
  **1.3배**로 셈하면 맞는다.
