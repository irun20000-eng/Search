...
■ 측정 공식 — 자수는 '본문만' 센다 (frontmatter 제외)
  ...
  ⚠ 2026-09-16 — **도해 대체글(alt)도 뺀다.** 대체글은 화면에 나오지 않는 접근성
    메타데이터라 산문이 아닌데, 도해를 붙일수록 분량을 밀어 올렸다
    (`episode-lebesgue-coins` 가 일화 상한 2,500 에 **4자**를 남기고 붙었다 — 2,496).
    그래서 **자수는 이제 `LC_ALL=C.UTF-8 wc -m` 과 다르다** — 대체글이 있는 노트에서는
    `wc -m` 이 더 큰 수를 준다. 공식은 `tools/mathlib.py` 의 `strip_alt` 머리말이 정본.
    완화되는 축을 밝혀 둔다 — **하한 쪽은 더 엄해지고(재는 값이 줄므로) 일화 상한이
    느슨해진다.** 하한을 새로 밑도는 노트가 없음을 95편 전수로 확인했다.
