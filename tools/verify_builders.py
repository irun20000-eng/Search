...
무엇을 보나 — 서가마다 **잡는 범위가 다르다.** 실측해서 적는다(2026-09-09).
    ...
    → **2026-09-09: 만들려고 재 봤더니 만들면 안 되는 것이었다**(손질된 카드 제목 16개·손으로 쓴
      tldr 72개를 덮는다). 대신 `tools/verify_reports_meta.py` 가 **소스가 있는 필드만**
      대조한다(date·depth·tags·**sources**·path·**cover**·양방향 고아 + **title 56편**).
      `sources` 는 사본 둘(소스수·manifest)에 더해 **실제 `## 출처` 목록**과 `tldr` 이 적은
      수까지 본다 — 사본끼리만 맞대면 **둘이 함께 틀렸을 때 초록이다**(그렇게 통과하던 2편을
      2026-09-09 2차 검수가 찾았다). 그 머리말에 무엇이 여전히 안 잡히는지 적혀 있다.
