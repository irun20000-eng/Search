...
## 시리즈 모드 (사용자 트리거: "시리즈로 작성해줘" / "시리즈형식으로", 확정 2026-06-23)
**질적 가치 > 단일 문서 규약.** 한 주제에 담을 게 많으면 단일 문서 강박을 버리고 **여러 파일 (1)(2)(3)…** 로 나눠 쓴다. 사용자가 **"시리즈"** 라고 명시하면 다음을 자동 수행한다:
1. **요청 메인 주제 1편 + 내가 선정한 파생·관련 주제 3편 = 총 4편 DEEP 시리즈**를 작성한다(파생 3편은 메인을 보완·확장하는 추천 주제로 내가 직접 선정해 **사전 보고**).
2. 각 편은 **독립적으로 DEEP 게이트를 통과**해야 한다(글자수·섹션·시각화·출처 하한 동일 적용; 실용형은 단일 DEEP 기준, 개념형은 투트랙 기준).
3. **시리즈 결속**: 모든 편 frontmatter에 `시리즈: "<시리즈명>"`·`시리즈순서: N/4`를 넣고, 본문 끝에 **시리즈 내비게이션**(◀이전 / ▶다음 + 전체 목차 위키링크)을 단다. 첫 태그 뒤에 `시리즈` 태그 추가.
4. slug는 공통 접두사로 묶는다(예: `claude-plugins-guide`, `claude-plugins-build`, …). 카드는 manifest 최상단에 **메인편이 위, 파생편이 아래로 연속** 배치.
5. **각 편마다 전체 파이프라인 완주**(작성→커밋→PR→머지→갤러리→옵시디언 `YYYYMMDD_제목`). 4편을 하나의 PR로 묶어도 되고 편별 PR도 가능하나, **모두 머지·저장 완료가 1건의 완료 정의**.
6. 분량이 너무 많으면 메인편부터 순차 완주하고, 진행 상황을 사용자에게 갱신한다("작성만 하고 멈추지 말 것" 유지).
...
**기준점(gold standard) = 이란-미국 보고서: 13,564자(`LC_ALL=C.UTF-8 wc -m`) / 186줄 / 15섹션.**

> 🚨 **단위 정정(2026-08-16) — 그동안 '자'로 적힌 값은 사실 '바이트'였다.**
> 이 컨테이너는 로케일이 비어 있어(`LANG=`) **로케일 없는 `wc -m`이 문자가 아니라 바이트를 센다**(한글 1자 = UTF-8 3바이트). 그래서 gold standard가 "25,104자"로 기록됐지만 **실제 글자수는 13,564자**(25,104는 바이트)였다.
> **다행히 잣대는 일관됐다** — 하한(15,000/20,000)도 gold standard의 **바이트** 값에서 뽑은 것이라, 지금까지의 합격/불합격 판정은 **공정한 비교**였다(LESSON 13의 '잣대 통일' 조건 충족). 틀린 것은 **숫자가 아니라 라벨**이다.
> **정정 방식**: gold standard 대비 **같은 비율을 유지**한 채 단위만 문자로 환산한다 — 이해편 15,000/25,104 = 59.8% → **≥ 8,000자**, 확장편 20,000/25,104 = 79.7% → **≥ 11,000자**. **게이트를 낮춘 것이 아니라 단위를 바로잡은 것**이며, 환산 근거를 위와 같이 공개한다.
> ⚠️ 측정 시 **반드시 `LC_ALL=C.UTF-8 wc -m`**을 쓸 것. 로케일을 빠뜨리면 바이트가 나와 **한글 문서가 실제보다 1.8배 길어 보인다**(수식·ASCII가 많을수록 배율이 낮아져 수학 문서가 상대적으로 불리해지는 왜곡도 있었다).

> 📏 **측정 지표 주의(2026-06-22 보정)**: 1차 지표는 **글자수 `LC_ALL=C.UTF-8 wc -m`**다. 한국어는 조사가 어절에 붙고 수학 문서는 수식·표 줄이 많아, `wc -w`(단어수)가 같은 분량의 서사형보다 **30~40% 낮게** 찍힌다(예: 이미 DEEP으로 인정된 hermes-deep는 742단어이나 본 벡터 확장편 1064단어보다 짧음). 그래서 단어수만으로 깊이를 판정하면 수학 DEEP을 부당하게 탈락시킨다 → **글자수·줄수를 정본 게이트**로, 단어수는 보조 참고로 둔다.

| 항목 | 이해편 `<concept>` | 확장편 `<concept>-advanced` |
|---|---|---|
| 대상 독자 | 고등학생 입문(본인 학습 노트) | 학부생~대학원생(이해편 전제) |
| **본문 글자수(`LC_ALL=C.UTF-8 wc -m`)** ★정본 | **≥ 8,000자** (기준의 ~60%) | **≥ 11,000자** (기준의 ~80%) |
| 본문 줄수(`wc -l`) | **≥ 140** | **≥ 165** |
| 본문 섹션(`##`) 수 | **≥ 8** | **≥ 10** |
...
```
LC_ALL=C.UTF-8 wc -m reports/<slug>/report.md           # ★정본: 글자수(로케일 필수! 빠뜨리면 바이트)
...
> **`갤러리URL` 은 쓰지 않는다** (2026-09-04 폐지). 갤러리 주소는 슬러그에서 기계적으로
> 유도되고, 볼트 배너는 `sync_obsidian.py` 가 manifest 슬러그로 **직접 짓는다**.
> reports 68편·math 78편 도합 146편에 적혀 있었으나 **저장소 전체에 읽는 코드가 없었다** —
> 걷어내고 빌더를 다시 돌렸을 때 산출물 7개가 비트 단위로 같았다. 다시 넣지 말 것.

본문 순서: `## TL;DR`(3~5줄) → `## 핵심 포인트` → `## 학습 카드` → `## 본문(섹션별)` → `## 출처(인용 링크)` → `## 후속 질문·연결([[위키링크]])`.
- **무환각**: 모든 사실 주장에 인용 [n]. 출처 없는 단정 0건. 해외 인용엔 한글 설명.
- **용어 주석(각주) 문법 — `[^라벨]`.** 어려운 개념·전문용어는 본문 흐름을 끊지 말고 각주로 뺀다. 본문에 `예후[^예후]`처럼 참조 → 문서 하단(출처 아래)에 `[^예후]: 질병의 경과에 대한 의학적 예측.`으로 정의. 갤러리가 이를 위첨자 번호 + 하단 "용어 주석" 섹션으로 자동 렌더(등장 순 번호, 클릭 점프, ↩ 되돌아가기). 옵시디언에서도 표준 각주로 동일하게 보임.
  - **`[^라벨]` = 용어·개념 주석**(설명), **`[n]` = 출처 인용**(사실 근거) — 둘은 별개 체계. 캐럿(`^`) 유무로 파서가 구분하므로 충돌 없음. 출처를 각주로 쓰지 말 것.
  - 참조되지 않은 정의는 렌더에서 제외됨(정의만 남기지 말 것). 특히 **개념 설명형 "이해편"** 에서 적극 활용.
