...
## 개념 설명서(딥) 포맷 (사용자 확정 2026-06-21, 최종)
**개념 설명·학습이 핵심인 주제**(수학/과학/이론 등)는 **수준별로 쪼개지 말고, 하나의 깊은 개념 설명서**로 쓴다. (앞선 '3계층 한 파일'·'레벨별 분리'는 둘 다 레벨당 얕아져 폐기.)
- **축은 '독자 수준'이 아니라 '개념의 깊이·확장 방향'.** 한 문서가 처음부터 끝까지 밀도 높게: 직관 → 정확한 정의 → **왜 그런가(유도)** → 대표 응용(완전히 푼 예제).
- **필수 섹션 「개념 발전 지도(확장 방향)」**: 이 개념이 *어떤 가정을 풀면 → 무엇이 새로 열리고 → 어디로 확장되나*를 **방향(direction)별**로 지도화(수준이 아니라 일반화 방향). 각 방향마다 "무엇을 더 알아야 / 무엇이 열리나 / 어디서 쓰나".
- 본문 권장 골격: TL;DR → 핵심 포인트 → 1.직관 → 2.정의·메커니즘 → 3.왜 그런가(유도) → 4.대표 응용(worked) → 5.역설·통합 의미 → 6.**개념 발전 지도** → 7.함정·체크리스트 → 출처 → 후속.
- 한 개념 = **카드 1장**(slug=`<concept>`). frontmatter `대상수준: 개념 설명서(딥)`. 옵시디언도 1파일 `YYYYMMDD_제목`.
- (시사·산업 동향형은 이 포맷 아님 — 기존 DEEP 타임라인/분석 기준.)
