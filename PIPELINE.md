...
**개념 설명·학습이 핵심인 주제**는 목적이 다른 **두 문서**로 낸다(수준이 아니라 *목적*이 축). 둘 다 깊게 쓰되 역할을 칼같이 나눠 **중복을 금지**한다.
| | 이해편 (입문·딥) | 확장편 (학부+대학원 통합) |
|---|---|---|
| slug | `<concept>` (대표) | `<concept>-advanced` |
| 목표 | 직관·납득 | 엄밀·일반화 |
| 핵심 | 비유·그림·작은 예제 풍부, "왜 자연스러운가", 대표 응용 1개 | 정확한 정의·유도/증명 스케치·여러 응용·**개념 발전 지도**·최전선 |
| 수식 | 말로 풀어서 | 기호·유도 그대로 |
| 전제 | 없음(맨바닥) | 이해편을 읽었다고 가정 |
| 끝맺음 | "더 엄밀히·확장은 → 확장편" 다리 | "직관이 헷갈리면 → 이해편" 역링크 |
- 한 개념 = **카드 2장**. frontmatter `대상수준: 이해편(입문·딥)` / `확장편(학부·대학원 통합)`. 옵시디언도 각각 `YYYYMMDD_제목`.
- **플래그십(비중 있는) 개념은 투트랙 기본**, 가벼운 개념은 단일 이해편 허용.
