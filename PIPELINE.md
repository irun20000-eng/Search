...
```
wc -w reports/<slug>/report.md                          # 단어수
grep -cE '^## ' reports/<slug>/report.md                # 섹션수
grep -cE '^\|.*\||^```' reports/<slug>/report.md         # 시각화(표행+코드블록)
grep -cE '^- \[' reports/<slug>/report.md               # 출처수
```
- 위 테이블 하한선을 **하나라도 못 넘으면 머지 금지** → 본문 보강 후 재측정.
- **PR 본문에 측정값 표 첨부 의무**(예: `단어 1320/≥1300 ✅ · 섹션 11/≥10 ✅ · 시각화 4/≥2 ✅ · 출처 13/≥12 ✅`).
