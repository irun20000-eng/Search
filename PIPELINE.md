...
## 학습형 보고서 = 3계층 누적 포맷 (사용자 지시 2026-06-21)
**개념 설명·학습이 핵심인 주제**(수학/과학/이론 등)는 한 보고서 안에 난이도를 쌓아 올린다:
- `🟢 Lv.1 입문(중·고등학생)`: 비유·그림·작은 수치 예제·"왜 배우나". 용어는 즉시 풀이.
- `🔵 Lv.2 학부`: 정확한 정의·정리·표준 표기·증명 스케치.
- `🟣 Lv.3 대학원/심화`: 일반화·추상화·인접 분야로의 확장·응용 최전선.
- 각 레벨 끝에 **"다음 레벨로 가는 다리"**(올라가려면 뭐가 더 필요한가)를 둔다. 학습 카드는 레벨별 한 줄.
- frontmatter `대상수준: 다층(입문→학부→대학원)`. (시사·산업 동향형은 이 포맷 아님 — 기존 DEEP 기준.)
