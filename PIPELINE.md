...
## 학습형 보고서 = 레벨별 분리 + 시리즈 인덱스 (사용자 확정 2026-06-21)
**개념 설명·학습이 핵심인 주제**(수학/과학/이론 등)는 레벨을 **각각 독립 파일로 풀뎁스** 작성하고, 짧은 **시리즈 인덱스(개념지도)**로 묶는다:
- `🟢 입문(중·고등)` / `🔵 학부` / `🟣 대학원/심화` — **각 레벨을 한 파일씩**, 예시·서술을 압축하지 말고 그 눈높이로 충분히.
- **시리즈 인덱스**(개념지도): 사다리 한눈 표 + 레벨 간 "다음 단계로 가는 다리" + 각 레벨 보고서 링크. 인덱스는 기존 slug 유지(예: `<concept>`), 레벨은 `<concept>-l1/-l2/-l3`.
- 슬러그 규칙: 인덱스=`<concept>`, 레벨=`<concept>-l1`(입문)·`-l2`(학부)·`-l3`(대학원).
- frontmatter: 레벨 파일은 `대상수준: 입문|학부|대학원`, 인덱스는 `대상수준: 시리즈 인덱스`.
- 한 개념 = 카드 4장(인덱스+3레벨). manifest에 모두 등록, 옵시디언도 각각 `YYYYMMDD_제목`으로 저장.
- (시사·산업 동향형은 이 포맷 아님 — 기존 DEEP 기준.)
